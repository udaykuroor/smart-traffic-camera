...
while True:
    ...
    # --- TRUCK DETECTION FIX ---
    # We explicitly tell model.track to use the yolo_device (GPU)
    results = model.track(
        frame,
        persist=True,
        conf=0.3,  # Lowered confidence to find more objects
        classes=[2, 3, 5, 7],
        tracker="bytetrack.yaml",
        imgsz=640, # Increased image size for better distant object detection
        half=(yolo_device.type == "cuda"),   # only use half precision if GPU available
        device=yolo_device,
        vid_stride=3,
        verbose=False
    )
    ...
    if len(results) > 0:
        ...
        for box in result.boxes:
            ...
            if resolved_cls == 2:   # Car class - needs sub-classification
                ...
                # Then reclassify every 15 frames to improve accuracy
                should_classify = (
                    ...
                    if car_crop.size > 0 and car_crop.shape[0] > 20 and car_crop.shape[1] > 20:
                        ...
                        if blur_var < BLUR_VAR_MIN:
                            ...
                            if probs_now is not None:
                                ...
                                # --- REMOVED SHAPE-BASED ADJUSTMENT BLOCK ---
                            ...
                            # Color detection (every COLOR_DETECTION_INTERVAL frames)
                            if frame_counter_per_id[track_id] % COLOR_DETECTION_INTERVAL == 0:
                                # NOW CALLS THE NEW V3.1 K-MEANS FUNCTION
                                color, color_conf = detect_vehicle_color(car_crop)
                                # Only store if not "unknown" or if it's first detection
            ...
            if resolved_cls == 2 and vehicle_type:
                # Get dominant color from recent detections (confidence-weighted)
                recent_colors = vehicle_data[track_id]['colors']
                if recent_colors:
                    # Look at last 5 detections
...
for track_id, data in vehicle_data.items():
    if len(data['positions']) < 5:   # Skip tracks with too few detections
        continue
    
    # Get final vehicle type
    vehicle_type = data['vehicle_type'] or "UNKNOWN"
    
    # Get dominant color (confidence-weighted)
    if data['colors']:
        ...
        if valid_colors:
            ...
            # Get color with highest weighted score
            dominant_color = max(color_weights, key=color_weights.get)
        else:
            dominant_color = "unknown"
    else:
    ...
    # Get average speed
    if data['speeds']:
        avg_speed = np.mean(data['speeds'][-10:])   # Average of last 10 measurements
    else:
        avg_speed = 0.0
